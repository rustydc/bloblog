...
from tinman.run import create_logging_node, create_playback_graph
...
class TestLoggingNode:
    ...
    async def test_create_logging_node(self, tmp_path):
        ...
        # Extract codecs from node specs
        specs = get_node_specs([producer])
        codecs = {}
        for spec in specs:
            for _, (channel_name, codec) in spec.outputs.items():
                codecs[channel_name] = codec
        
        # Create and run with logging node
        logger = create_logging_node(tmp_path, codecs)
    ...
    async def test_logging_node_with_filter(self, tmp_path):
        ...
        # Extract codecs
        specs = get_node_specs([producer_a, producer_b])
        codecs = {}
        for spec in specs:
            for _, (channel_name, codec) in spec.outputs.items():
                codecs[channel_name] = codec
        ...
        logger = create_logging_node(tmp_path, codecs, channel_filter={"channel_a_output"})
...
class TestPlayback:
    ...
    async def test_playback_from_logs(self, tmp_path):
        ...
        specs = get_node_specs([recording_producer])
